<div class="row"><div class="col-md-12"><div class="card card-primary">
    ...
    <div class="card-body">
        <dl class="row">
            <dt class="col-sm-3">Pesanan</dt><dd class="col-sm-9"><?php echo html_escape($retur->pesanan->nomor_pesanan); ?> <span class="text-muted">(total Rp <?php echo number_format((float) $retur->pesanan->total, 0, ',', '.'); ?>, <?php echo html_escape($retur->pesanan->status_bayar); ?>)</span></dd>
            <dt class="col-sm-3">Pasien</dt><dd class="col-sm-9"><?php echo html_escape($retur->pasien ? $retur->pasien->nama : '-'); ?> <?php echo ($retur->pasien && $retur->pasien->no_hp) ? '(' . html_escape($retur->pasien->no_hp) . ')' : ''; ?></dd>
            <dt class="col-sm-3">Diajukan</dt><dd class="col-sm-9"><?php echo html_escape($retur->tanggal_pengajuan); ?></dd>
            <?php if (! empty($retur->tanggal_keputusan)): ?>
            <dt class="col-sm-3">Diputus</dt><dd class="col-sm-9"><?php echo html_escape($retur->tanggal_keputusan); ?></dd>
            <?php endif; ?>
        </dl>
    ...
    <div class="card-footer">
        ...
        <span class="float-right">
            <button type="submit" name="aksi" value="tolak" class="btn btn-danger" onclick="return confirm('Tolak pengajuan retur ini? Catatan wajib terisi.')"><i class="fas fa-times"></i> Tolak</button>
            <button type="submit" name="aksi" value="setujui" class="btn btn-success" onclick="return confirm('Setujui retur? Stok RESTOCK bertambah dan refund tercatat. Tidak dapat dibatalkan.')"><i class="fas fa-check"></i> Setujui</button>
        </span>
        <?php endif; ?>
    </div>
