$('#dokter_table').bfDataTable({
    ...
    columns: [
        ...
        { data: null, orderable: false, searchable: false, render: function(data) {
            var editUrl = site_url + 'admin/master/dokter/edit/' + data.id;
            var deleteUrl = site_url + 'admin/master/dokter/delete/' + data.id;
            return '<a href="' + editUrl + '" class="btn btn-xs btn-primary mr-1"><i class="fas fa-edit"></i></a>' +
                '<button class="btn btn-xs btn-danger btn-hapus" data-url="' + deleteUrl + '" data-nama="' + data.nama_dokter + '"><i class="fas fa-trash"></i></button>';
        } }
    ]
});
$('.select2').select2();
        ...
        if (result.isConfirmed) {
            $.post(url, function(res) {
                if (res.success) {
                    ...
                    $('#dokter_table').DataTable().ajax.reload(null, false);
                } else {
