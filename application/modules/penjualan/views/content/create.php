...
<div class="row"><div class="col-md-12"><div class="card card-primary">
    ...
    <?php echo form_open($this->uri->uri_string()); ?><div class="card-body">
        <div id="alert_stok_cukup" class="alert alert-warning mb-3" style="display:none;"><i class="fas fa-exclamation-triangle mr-2"></i> <strong>Peringatan Stok Tidak Cukup!</strong> Terdapat jumlah obat yang melebihi stok tersedia. Tombol simpan dinonaktifkan.</div>
        <div class="row">
        ...
        <div id="item_rows">
            <div class="row item-row mb-2">
                <div class="col-md-6"><select name="id_obat[]" class="form-control select2 input-id-obat" required><option value="">-- Pilih Obat --</option><?php foreach ($obat_list as $o): ?><option value="<?php echo $o->id_obat; ?>" data-stok="<?php echo (int) $o->stok; ?>"><?php echo html_escape($o->nama_obat . ' (Stok: ' . (int) $o->stok . ')'); ?></option><?php endforeach; ?></select><small class="form-text text-muted info-stok-text"></small></div>
                <div class="col-md-4"><input type="number" min="1" name="jumlah[]" class="form-control input-jumlah" placeholder="Jumlah" required><small class="text-danger err-stok-text" style="display:none;"></small></div>
    ...
    </div><div class="card-footer"><button type="submit" name="save" id="btn_submit" value="1" class="btn btn-primary">Simpan Penjualan</button><a href="<?php echo site_url(SITE_AREA . '/' . $this->uri->segment(2) . '/' . $this->uri->segment(3)); ?>" class="btn btn-default float-right">Batal</a></div><?php echo form_close(); ?>
</div></div></div>
<script type="text/javascript">var daftarObat = <?php $o = array(); foreach ($obat_list as $x) { $o[] = array('id' => (int) $x->id_obat, 'nama' => $x->nama_obat, 'stok' => (int) $x->stok); } echo json_encode($o); ?>;</script>
