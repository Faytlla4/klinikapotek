...
class BF_Form_validation extends CI_Form_validation
{
	...
	/**
	 * Check that a value is unique in the database.
	 *
	 * Override CI_Form_validation::is_unique(): CI memakai isset($this->CI->db)
	 * yang selalu FALSE pada stack HMVC ini (db tersedia via __get), sehingga
	 * semua rule is_unique selalu gagal. Versi ini mengakses db langsung.
	 *
	 * @param string $str   Nilai yang dicek.
	 * @param string $field Format "tabel.kolom".
	 * @return bool True bila belum ada di database.
	 */
	public function is_unique($str, $field)
	{
		...
		return $db->limit(1)->get_where($table, array($field => $str))->num_rows() === 0;
	}
