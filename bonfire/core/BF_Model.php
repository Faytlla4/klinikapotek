...
class BF_Model extends CI_Model
{
	...
	public function __construct()
	{
		...
		if (!isset($this->db) && isset($this->load)) {
			$this->load->database();
		}
	...
	public function insert_batch($data = null)
	{
		...
		if (!empty($set)) {
			foreach ($data as $key => &$record) {
				...
				$data[$key] = array_merge($set, (array)$data[$key]);
			}
		}
	...
	public function validate($data, $type = 'update')
	{
		...
		// ponytail: form_validation dipakai bersama antar model dalam satu request;
		// reset agar rule/error dari validasi model lain tidak terbawa.
	...
	protected function set_date($user_date = null)
	{
		...
		$time_ref = strtolower(isset($this->config) && is_object($this->config) ? $this->config->item('time_reference') : 'local');
		return ($time_ref == 'gmt' ?
