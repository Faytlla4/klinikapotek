...
class Permission_model extends BF_Model
{
    ...
    {
        $perm = $this->find_by('nama_permission', $name);
        if (empty($perm) || ! isset($perm->id_permission)) {
            return false;
        }

        return $this->delete($perm->id_permission);
    }
    ...
    public function permission_exists($permission = null)
    {
        ...
        if ($this->find_by('nama_permission', $permission)) {
            return true;
        }
