...
class Role_permission_model extends BF_Model
{
    ...
    public function assign_to_role($role_name = null, $permission_name = null)
    {
        ...
        $role = $this->role_model->where('deleted', 0)->find_by('nama_role', $role_name);

        if (! class_exists('permission_model', false)) {
            $this->load->model('permissions/permission_model');
        }
        $perm = $this->permission_model->find_by('nama_permission', $permission_name);
    ...
    {
        // ponytail: skema apotek memakai id_role/id_permission
        return $this->select('id_permission AS permission_id')->where('id_role', $role_id)->find_all();
    }
